...
import PIDB from '../../../../packages/workbox-core/_private/indexedDBHelper.mjs';
...
describe(`PIDB`, function() {
  ...
  before(async function() {
    ...
    const db = await new PIDB('db', 1).open();
  ...
  describe(`constructor`, function() {
    ...
    it('lets you specify callbacks', function() {
      ...
      const db = new PIDB('db', 1, {onupgradeneeded, onversionchange});
  ...
  describe(`open`, function() {
    ...
    it(`returns the instance for easier init + assignment`, async function() {
      const db = await new PIDB('db', 1).open();
      expect(db).to.be.instanceOf(PIDB);
    ...
    it(`runs the onupgradeneeded callback if specified`, async function() {
      ...
      const db = await new PIDB('db', 1, {onupgradeneeded}).open();
    ...
    it(`sets the onversionchange callback`, async function() {
      const onversionchange = () => {};
      const db = await new PIDB('db', 1, {onversionchange}).open();
    ...
    it(`throws if blocked for more than the timeout period when openning`,
      ...
      sandbox.stub(PIDB.prototype, 'OPEN_TIMEOUT').value(100);
      ...
      const err = await catchAsyncError(new PIDB('db', 2).open());
    ...
    it(`times out even in cases where the onblocked handler doesn't file`,
      ...
      const err1 = await catchAsyncError(new PIDB('db', 2).open());
      const err2 = await catchAsyncError(new PIDB('db', 2).open());
  ...
  describe(`find`, function() {
    it(`returns all entries in an object store by default`, async function() {
      ...
      for (let i = 0; i < data.users.length; i++) {
        expect(users[i].value).to.deep.equal(data.users[i]);
        expect(users[i].key).to.equal(data.users[i].email);
        expect(users[i].primaryKey).to.equal(data.users[i].email);
      }

      const posts = await db.find('posts');
      for (let i = 0; i < data.posts.length; i++) {
        expect(posts[i].value).to.deep.equal(data.posts[i]);
        expect(posts[i].key).to.equal(i + 1);
        expect(posts[i].primaryKey).to.equal(i + 1);
      }

      const comments = await db.find('comments');
      for (let i = 0; i < data.comments.length; i++) {
        expect(comments[i].value).to.deep.equal(data.comments[i]);
        expect(comments[i].key).to.equal(i + 1);
        expect(comments[i].primaryKey).to.equal(i + 1);
      }

      await db.deleteDatabase();
    });

    it(`accepts options to customize the find options`, async function() {
      const db = await createAndPopulateTestDb();
      ...
      expect(user.value).to.deep.equal(lastUser);
      expect(user.key).to.equal(lastUser.email);
      expect(user.primaryKey).to.equal(lastUser.email);
      ...
      const lastUserPostIndexes = [2, 3, 5];
      for (let i = 0; i < lastUserPostIndexes.length; i++) {
        expect(posts[i].value).to.deep.equal(
            data.posts[lastUserPostIndexes[i]]);
        expect(posts[i].key).to.equal(
            data.posts[lastUserPostIndexes[i]].userEmail);
        expect(posts[i].primaryKey).to.equal(lastUserPostIndexes[i] + 1);
      }

      // Gets the last comment from the first post
      const [comment] = await db.find('comments', {
        index: 'postId',
